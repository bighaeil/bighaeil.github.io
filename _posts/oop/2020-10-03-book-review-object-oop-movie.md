...
영화 예메 시스템에 대한 기본적인 정보와 용어는 전 포스트를 확인  
...
> 어떤 `클래스(class)`를 필요한지 고민하기 전에 어떤 객체들이 필요한지 고민하라.  
>  객체는 홀로 존재하는 것이 아니다.
...
즉 어떤 부분은 공개하고 어떤 부분 감출 것인지 결정하는 것인데 이는 객체의 직접적인 접근을 막을 수 있다.  
즉 객체의 자율성을 보장하기 위함이다.  

객체의 상태(state)와 행동(behavior)을 묶은 것을 `캡술화`라고 부른다.  
이런 캡슐화에 접근을 통제할 수 있는게 `접근 제어(access control)`이다.  
대부분의 객체지향 프로그래밍은 이를 제공하는 `접근 수정자(access modifier)`를 제공한다.  

외부에서 접근이 가능한 부분을 `퍼블릭 인터페이스(public interface)`로  
접근 불가능한 부분을 `구현(implementation)`으로  
...
이하 요금과 관련된 명칭 다양하게 있을 예정인데 전부 `요금` 객체라고 생각하면 되겠다. 이는 스크립트를 단순화 하기 위한 구현 생략이라고 판단하자.  
```java
class 요금 {
...
(`static`은 나중에 다시 확인해야 겠다. 일단은 객체가 어딘가에 있어서 그냥 사용가능이라고 이해)
...
객체는 다른 객체의 공개된 인터페이스를 통해 `요청(request)`을 할 수 있다.  
요청받은 객체는 이를 처리하고 `응답(response)`을 한다.  
이것이 협력의 기본이다.  
...
`메시지(message)`는 전송하거나 수신하여 객체가 어떻게 처리할지를 결정하게 하는 것  
수신된 메시지를 처리할 자신만의 방법이 `메서드(method)`  
...
영화 객체의 메소드를 보면 요금을 계산하지만 어떤 할인 정책을 선택하는지는 보이지 않는다.  
단지 `할인정책`에 메시지(요금을_할인하다)를 전달할 뿐이다.  
...
그래서 부모 클래스인 `할인정책` 안에 중복 코드를 두고 `금액할인정책`과 `비율할인정책`이 상속 받도록 한다.  
...
`할인정책`은 할인조건(조건들)을 여러개 가질 수 있다.  
`할인조건`을 만족하면 해당 `요금`을 얻을 수 있다.  
`추상 메서드`를 호출하여 할인 요금을 계산한다.  

전체흐름은 부모 클래스(할인정책)이 정의하지만 실제 요금을 계산하는 것은 추상 메서드(할인된_요금은)가 될 것이다. 이는 부모 클래스를 상속받아 오버라이딩한 자식 클래스(금액할인정책, 비율할인정책)가 구현한 정의에 의해 결정된다.  
...
그러면 할인정책에서 요구하는 형식에 맞는 인터페이스를 사용할 수 있다.  
이 인터페이스를 가져와 구현하여 조건이 만족하는지 확인할 수 있다.  
...
`할인조건` 인터페이스를 사용하면서 `만족하면(상영)` 메서드를 구현했다.  
여기서 할인조건은 이번 상영이 할인이 되는지 아닌지를 판단하는 도구이다.  
...
`할인정책`을 상속받아서 할인된 요금 정책을 구현했다.  
돈과 관련된 할인하는 값(금액할인, 할일율)은 `요금` 객체로 구현하니 신경쓰지 말자.  

위로 올라가 `영화` 객체를 확인하면  
영화당 하나의 `할인정책`을 받는다.  
그리고 `할인정책`의 `조건들`은 복수 할인 조건을 넣을 수 있다.  

이제 실제 영화(아바타, 타이타닉, 스타워즈)를 구현하면  
...
영화를 확인하기 위해 상속이란 개념을 확인하자.


## 상속

상속은 부모 클래스의 모든 인터페이스를 자식 클래스가 물려받아 사용할 수 있도록 한다.  
그래서 외부 객체는 자식 클래스를 부모 클래스와 동일하게 간주할 수 있다.  

`영화`의 `요금을_게산하다`를 보면 `할인정책`의 `요금을_할인하다` 인터페이스만 구현되어 있으면 어떤 자식이 와도(어떻게 구현되어 있든간에) 부모 클래스의 메소드를 사용할 수 있다.  
...
`영화`는 동일한 메시지를 전송하지만 실제 메시지를 수행하는 객체는 클래스가 무엇이냐에 따라 달라진다.  
이것을 `다형성`이라고 한다.

> 다형성이란 동일한 메시지를 수신했을 때 객체의 타입에 따라 다르게 응답할 수 있는 능력

`영화` 클래스는 추상클래스 `할인정책`에 의존한다.  
그러나 실제로 상호작용하는 `할인정책` 객체는 `금액할인정책`이나 `비율할인정책`의 인스턴스다.

> 다형성은 컴파일 시간 의존성과 실행 시간 의존성을 다르게 만들 수 있는 객체지향의 특성을 이용해 서로 다른 메서드를 실행할 수 있게 한다.


## 다형성의 컴파일 시점과 실행 시점 용어

실행될 메서드를 실행 시점에 결정하는 것을 - 실행 시간 의존성

> `지연 바인딩(lazy binding)` 또는 `동적 바인딩(dynamaic binding)`

이라 부르고  

전통적인 함수 호출처럼 컴파일 시점에 실행될 함수를 결정하는 것을 - 코드의 의존성
...
## 다형성의 컴파일 시점과 실행 시점의 트레이드 오프

코드의 의존성과 실행 시점 의존성이 다르면

> 코드가 유연해짐
> 쉽게 재사용 가능
> 확장 가능

코드의 의존성과 실행 시점 의존성이 같으면
...
예제에서 추상적인 개념을 사용한 것은 `할인정책`과 `할인조건`이다.

`할인정책`은 추상 클래스를 사용했고  
`할인조건`은 인터페이스를 사용했다.

프로그래밍 언어 측면에서 `할인조건`이 `할인정책`보다 더 추상적이다.  
둘다 자식 클래스가 정의할 수 있도록 결정권을 위임한다. 하지만 인터페이스가 더 자식의 결정에 전적으로 위임하기 때문이다.~~(이유는 각자 생각)~~
...
일관성을 유지하면서 유연한 설계를 하기위해 어떻게 해야할까?

아까 정의하지 않은 스타워즈를 다시 보자.

`영화` 스타워즈는 할인 정책이 적용돼 있지 않다. 이런 "할인 정책이 없는 경우"는 예의 케이스로 취급하기 때문에 지금까지 일관성 있던 협력 방식이 무너지게 된다.
...
`할인정책`을 상속받는 `할인없는정책`을 구현하면 일관성 있는 설계를 할 수 있다.
...
`할인정책`을 다시 살펴보자. `요금을_할인하다(상영)`는 `할인조건`이 없으면 `할인된_요금은(상영)`을 실행하지 않고 `요금.ZERO`를 반환한다. 이는 `할인정책`에 `할인없는정책`의 개념이 포함되어 있는 설계이다. 그래서 이를 더 명확히 수정해보자.
...
`할인없는정책`을 위해 `할인정책`을 인터페이스로 바꾸고 다른 클래스들도 수정되는 것이 과하게 느껴질 수도 있지만 어쨌든 0원을 효과적으로 전달하는 것은 사실이다.
...
`영화`에서 `할인정책`을 합성으로 사용하고 있다. 이 설계를 상속으로 사용할 수도 있는데 `할인정책`에서 `영화`를 직접 상속받아 `금액할인영화` 또는 `비율할인영화` 클래스를 추가하면 기존의 방법과 기능적으로 동일하다.  
...
즉 부모 클래스의 구현이 자식 클래스에게 그대로 노출되기 때문에 캡슐화가 약하게 된다.  
즉 코드 변경이 어려워진다.  

> 설계를 유연하지 못하게 만든다

부모 클래스와 자식 클래스 사이의 관계가 컴파일 시점에서 결정된다. 이러면 실행 시점에 객체의 종류를 변경하기 힘들어진다. 그에 반해 인스턴스 변수를 사용하면 그냥 새로운 `할인정책`을 넣어주면 간단히 해결된다.  
즉 상속보다 합성이 더 유연한 설계가 가능한는 결론이다.
